...
class SocialGraph:
    ...
    def populate_graph(self, num_users, avg_friendships):
        ...
        for user_id in self.users:
            # add one to user_id(zero based index) to align with user (1 based)
            for friend_id in range(user_id +1, self.last_id +1):
                possible_friendships.append((user_id, friend_id))
        random.shuffle(possible_friendships)
        # divide number of friendships by two (because friendships go two ways)
    ...
    def get_all_social_paths(self, user_id):
        ...
        while neighbors_to_visit.size() > 0:
            ...
            if current_vertex not in visited:
                ...
                # add the current vertex to the visited dictionary and the path that led there
